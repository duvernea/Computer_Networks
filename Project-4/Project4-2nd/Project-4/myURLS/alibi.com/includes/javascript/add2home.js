...
var NON_IOS_HACK=false; // for debugging outside of iOS (inspect element!)
...
var addToHome = (function (w) {
	var nav = w.navigator,
		...
		options = {
			...
			titleReplace: '',			// ALIBI replace page title for better default iOS bookmark title
			iuiWorkaround: false		// ALIBI inline-style to make stuff visible despite crappy iui css
		},
	...
	function init () {
		...
		if ( !isIDevice && !NON_IOS_HACK) return;
	...
	function loaded () {
		...
		if (options.iuiWorkaround)
			balloon.style.cssText += 'display:block; min-height: 84px;' // override iui crap
	...
	function show () {
		...
		if (options.titleReplace)
			document.title=options.titleReplace;
	...
	function manualShow (override) {
		if ( !(isIDevice||NON_IOS_HACK) || balloon ) return;
